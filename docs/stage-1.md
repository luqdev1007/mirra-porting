...
- `Tools/Mirra Porting/Scan and Export` — без диалогов, без ожидания ввода и без кнопки отмены: этот путь вызывает агент, нажимать отмену некому. Прогресс-бар заводится здесь, а не в сканере, и закрывается в `finally`.
- Строка в консоль включает editor-only отдельным числом: `Scan finished: 20 errors, 17 warnings, 0 info, 1 editor-only, 12 files, 51 ms → Library/MirraPorting/report.json`. Путь печатается относительно корня проекта.
- При `null` от `Export` пункт меню пишет `Scan failed` вместо строки об успехе; подробности (путь и причина) уже залогированы самим `ScanAndExport`.
- Проверено на реальном проекте вызовом пункта меню через MCP.

### Шаг 6, автосверка песочницы

- `Assets/SandboxTests/` — отдельная сборка в проекте разработки, не в пакете: она проверяет пакет снаружи, только через публичный `MirraPortingScanner`, и в порты не переносится. asmdef: только Editor, `UNITY_INCLUDE_TESTS`, `autoReferenced: false`.
- Тест запускает скан во временную папку, читает `report.json` **своим** DTO (проверяется файл, а не согласие сканера с самим собой) и сравнивает множества с ожиданиями из пометок `// EXPECT` в `Assets/Sandbox`. При расхождении печатает обе разницы — чего не хватает и что лишнее.
- Сравниваются только находки с префиксом `Assets/Sandbox/` (со слэшем): `Assets/SandboxTests/X.cs` и `Assets/Game/Sandbox/A.cs` не проходят фильтр, на это есть отдельный тест. Плюс проверка, что находок из `Packages/com.luqmus.mirra-porting` нет вовсе.
- Проверено, что сверка умеет падать: временная порча одного `EXPECT` (Error → Warning) валит тест и печатает обе строки разницы. После проверки ожидание возвращено.
- `EXPECTED.md` остаётся человекочитаемым эталоном и генерируется из тех же пометок; расхождение теперь ловит тест, а не ручная сверка.

### Закрытие этапа 1

- **Шаг 7 (`ScannerWindow`) отложен.** Для работы через агента достаточно пункта меню и `report.json` / `report.md`: агент читает JSON, человек — Markdown. Окно вернём после первого реального порта, когда станет видно, чего в отчёте не хватает глазами. Пункт меню `Tools/Mirra Porting/Scanner` тоже не сделан.
- **Критерий приёмки 6 (окно: скан, фильтры, переход к строке, экспорт, восстановление после domain reload) отложен вместе с шагом 7.**
- **Критерий приёмки 7 изменён:** установка в пустой проект в `$env:TEMP` не проверяется, вместо этого пакет проверяется на реальном порте. Здесь проверено только то, что `install.ps1` и `uninstall.ps1` актуальны после шагов 2–6: оба работают с папкой пакета целиком (`Copy-Item -Recurse`, `Remove-Item -Recurse`), списков файлов внутри нет. Копирование в пустышку: 236 файлов исходника и 236 скопированных, наборы совпадают, новые папки `Checks`, `Code`, `Core`, `Report`, `UI` на месте, удаление чистое.
- Версия пакета остаётся `0.1.0`; на закрывающий коммит поставлен тег `v0.1.0`.
- Тест-«вредитель» для ветки «проверка упала» решено не делать: ветка тривиальная и проверена чтением, а живой сбой лексера воспроизвести строкой нельзя — он на то и не бросает.
- `EXPECTED.md` остаётся, хотя автосверка делает то же самое: файл читаем глазами и служит эталоном при разборе расхождений.

### Состояние на конец этапа 1

Готово: скелет пакета и скрипты установки, модель находок и правил, область сканирования, лексер C# с условной компиляцией и using-контекстом, `ForbiddenApiCheck` по таблице запрещённых API, `ScanRunner`, отчёты JSON и Markdown, пункт меню, песочница с автосверкой. Тестов 344, все зелёные; консоль Unity без ошибок и предупреждений от `Luqmus.*`.

Не сделано намеренно: окно сканера (шаг 7) и всё из раздела «Не входит в этап 1».
